...
class MaskTransformer(nn.Module):
    ...
    def trans_forward(self, motion_ids, cond, padding_mask, force_mask=False):
        '''
        :param motion_ids: (b, seqlen)  # (64, 49)
        :padding_mask: (b, seqlen), all pad positions are TRUE else FALSE  # (64, 49)
        :param cond: (b, embed_dim) for text, (b, num_actions) for action  # (64, 512)
        :param force_mask: boolean
        :return:
            -logits: (b, num_token, seqlen)    # (64, 512, 49))
        '''

        cond = self.mask_cond(cond, force_mask=force_mask) # (64, 512) # 在训练时按概率随机丢弃一些 batch 的条件（全0）

        # print(motion_ids.shape)
        x = self.token_emb(motion_ids)  # (b, n, latent_dim)  # (64, 49, 512)
        # print(x.shape)
        # (b, seqlen, d) -> (seqlen, b, latent_dim)
        x = self.input_process(x)  #  (seqlen, b, latent_dim)  # (49, 64, 512)

        cond = self.cond_emb(cond).unsqueeze(0) #(1, b, latent_dim)  # (1, 64, 512)


        x = self.position_enc(x)
        xseq = torch.cat([cond, x], dim=0) #(seqlen+1, b, latent_dim)  # (50, 64, 512)

        padding_mask = torch.cat([torch.zeros_like(padding_mask[:, 0:1]), padding_mask], dim=1) #(b, seqlen+1) # (64, 50)
        # print(xseq.shape, padding_mask.shape)

        # print(padding_mask.shape, xseq.shape)

        output = self.seqTransEncoder(xseq, src_key_padding_mask=padding_mask)[1:] #(seqlen, b, e)  # (49, 64, 512)
        logits = self.output_process(output) #(seqlen, b, e) -> (b, ntoken, seqlen)  # (64, 512, 49) 
        return logits
    ...
    def forward(self, ids, y, m_lens):
        ...
        :return:
        '''
        bs, ntokens = ids.shape  # (b, n)  # (64, 49)
        device = ids.device

        # Positions that are PADDED are ALL FALSE
        non_pad_mask = lengths_to_mask(m_lens, ntokens)  # (b, n)
        ids = torch.where(non_pad_mask, ids, self.pad_id) # (b, n)  # pad_id: 513

        force_mask = False
        if self.cond_mode == 'text':
            with torch.no_grad():
                cond_vector = self.encode_text(y)  # (64, 512)
        elif self.cond_mode == 'action':
        ...
        labels = torch.where(mask, ids, self.mask_id)  # debug: mask_id = 514
        ...
        logits = self.trans_forward(x_ids, cond_vector, ~non_pad_mask, force_mask)  # 细节 non_pad_mask 取反
...
class ResidualTransformer(nn.Module):
    ...
    def mask_cond(self, cond, force_mask=False):
        bs, d =  cond.shape  # (64, 512)
        if force_mask:
            return torch.zeros_like(cond)
        elif self.training and self.cond_drop_prob > 0.:
            # debug: self.cond_drop_prob = 0.1
            mask = torch.bernoulli(torch.ones(bs, device=cond.device) * self.cond_drop_prob).view(bs, 1)  # (bs, 1)  # (64, 1)
            return cond * (1. - mask) # (64, 512)
        else:
            return cond
